package icu.takeneko.plugins.crystal.backup.command
...
import icu.takeneko.plugins.crystal.backup.file.*
import icu.takeneko.plugins.crystal.backup.logResponse
import icu.takeneko.plugins.crystal.backup.requirePermission
import java.io.File
...
object BackupCommand {
    ...
    private val helpTexts = TextGroup(
        Text(Config.config.commandPrefix + "backup -> Show help info"),
        Text(Config.config.commandPrefix + "backup slot create <slot> -> Create a new backup slot"),
        Text(Config.config.commandPrefix + "backup slot delete <slot> -> Delete a backup slot "),
        Text(Config.config.commandPrefix + "backup slot info <slot> -> Show slot info"),
        Text(Config.config.commandPrefix + "backup slot list -> List all slot details"),
        Text(Config.config.commandPrefix + "backup make [<slot>] [<comment>]-> Make a backup to <slot> with a <comment>"),
        Text(Config.config.commandPrefix + "backup restore [<slot>] -> Restore world from <slot>"),
        Text(Config.config.commandPrefix + "backup confirm -> Confirm a operation"),
        Text(Config.config.commandPrefix + "backup abort -> Abort a operation."),
        Text("*Any arguments wrapped with [] is optional*")
    )

    fun register(): LiteralArgumentBuilder<CommandSourceStack> {
        return literal(Config.config.commandPrefix + "backup").then(
            literal("slot").then(
                literal("delete").then(
...
//            ClickEvent(ClickAction.suggest_command, "${Config.config.commandPrefix}backup make")
//        )))
    }
                ...
                .append {
                    if (!it.isEmpty)
                        text(" [▶]", NamedTextColor.GREEN).clickEvent(
                            ClickEvent.clickEvent(
                                ...
                                "${Config.config.commandPrefix}backup restore ${it.id}"
                            )
                        ).hoverEvent(
                            HoverEvent.showText(
                                Component.text(
                        ...
                        text(" [▶]", NamedTextColor.GREEN).clickEvent(
                            ClickEvent.clickEvent(
                                ClickEvent.Action.SUGGEST_COMMAND,
                                "${Config.config.commandPrefix}backup make ${it.id}"
                            )
                        ).hoverEvent(
                            HoverEvent.showText(
                                Component.text(
                    ...
                    if (!it.isEmpty)
                        Component.text(" [×]", NamedTextColor.RED).clickEvent(
                            ClickEvent.clickEvent(
                                ClickEvent.Action.SUGGEST_COMMAND,
                                "${Config.config.commandPrefix}backup slot clear ${it.id}"
                            )
                        ).hoverEvent(
                            HoverEvent.showText(
                                Component.text(
        ...
        if (SlotManager.scheduledProcedure == null) {
            synchronized(SlotManager) {
                ...
                if (SlotManager.config.requireConfirm) {
                    commandSourceStack.sendFeedback(text("Type \"${Config.config.commandPrefix}backup confirm\" in 5 minutes to confirm operation."))
                } else {
                    SlotManager.scheduledProcedure!!.start()
                }
            }
